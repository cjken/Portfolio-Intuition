---
title: "Portfolio intuition"
author: | 
  | Chris Kennedy
  | Bridge Alternatives
  | chris.kennedy@bridgealternatives.com
...
```{r R Imports and configurations, include=FALSE, tidy=FALSE}
library(reticulate)
library(scales)

use_python("/Users/chriskennedy/anaconda3/bin/python")
```
...
from sympy import symbols, sqrt, Eq, solve, lambdify, diff
from matplotlib.cbook import MatplotlibDeprecationWarning
```

```{python Matplotlib configuration, include=FALSE}
warnings.simplefilter('ignore', MatplotlibDeprecationWarning)

# Custom mayplotlib styling
plt.style.use('~/.jupyter/custom/matplotlib/custom.mplstyle')
```

```{python Labeling function, include=FALSE}
def label_lines(lines, ax, xlocs, vas):
    for i, line in enumerate(lines):
        label = line.get_label()
        xs = line.get_xdata()
        ys = line.get_ydata()
        
        # Find a near x location and interpolate the y location
        j = np.max([k if x < xlocs[i] else 0 for k, x in enumerate(xs)])
        xloc = xlocs[i]
        yloc = ys[j - 1] + (ys[j] - ys[j - 1]) * (xloc - xs[j - 1]) / (xs[j] - xs[j - 1])
        
        # Calculate the slope at that location
        dx = xs[j] - xs[j - 1]
        dy = ys[j] - ys[j - 1]
        ang = degrees(atan2(dy, dx))
        
        # Transform to coords
        point = np.array([[xloc, yloc]])
        rotation = ax.transData.transform_angles(np.array((ang,)), point)[0]
...
```{python Core and implied functions, include=FALSE}
def two_asset_return(r1, r2, w1, w2):
    return (r1 * w1) + (r2 * w2)

def two_asset_risk(s1, s2, p12, w1, w2):
    return (
        (w1**2 * s1**2) + (w2**2 * s2**2) + (2 * s1 * s2 * w1 * w2 * p12)
    )**(1/2)

def two_asset_ratio(r1, r2, s1, s2, p12, w1, w2):
    return two_asset_return(r1, r2, w1, w2) / two_asset_risk(s1, s2, p12, w1, w2)
    
def implied_r2(ratio, r1, s1, s2, p12, w1, w2):
    return (ratio * two_asset_risk(s1, s2, p12, w1, w2) - (w1 * r1)) / w2

def implied_p12(ratio, r1, r2, s1, s2, w1, w2):
    rp = two_asset_return(r1, r2, w1, w2)

    return (
        ((rp / ratio)**2 - (w1**2 * s1**2) - (w2**2 * s2**2)) /
        (2 * w1 * w2 * s1 * s2)
    )

def implied_s2(ratio, r1, r2, s1, p12, w1, w2):
    # quadradic formula parameters
    a = w2**2
    b = 2 * w1 * w2 * s1 * p12
    c = (w1**2 * s1**2) - (((w1 * r1) + (w2 * r2)) / ratio)**2
    
    # compute the portion under square root to determine if it's 
    # negative; we don't want complex solutions
    u = (b**2 - 4 * a * c)
    ...
    if u < 0:
        ...
        return sorted([s2a, s2b])
```

```{python Asset definitions, include=FALSE}
class Asset:
    def __init__(self, rtn=None, risk=None, corr=None):
        self.rtn = rtn
        self.risk = risk
        self.corr = corr
        
# Portfolio
port = Asset(rtn=0.05, risk=0.15)

# Ratio
ratio = (port.rtn / port.risk) * 1.10 # 10% increase
...
# A ==========
# Different risks
a1 = Asset(rtn=0.04, corr=-0.2)
a2 = Asset(rtn=0.04, corr=-0.2)

a1.risk, a2.risk = implied_s2(ratio, port.rtn, a1.rtn, port.risk, a1.corr, w1, w2)

# B ==========
# Different returns and correlations
b1 = Asset(risk=0.20, corr=0.8)
b2 = Asset(risk=0.20, corr=-0.2)

b1.rtn = implied_r2(ratio, port.rtn, port.risk, b1.risk,b1.corr, w1, w2)
b2.rtn = implied_r2(ratio, port.rtn, port.risk, b2.risk, b2.corr, w1, w2)

# C ==========
# Different returns and risks
c1 = Asset(corr=0.4, risk=0.275)
c2 = Asset(corr=0.4, risk=0.125)

c1.rtn = implied_r2(ratio, port.rtn, port.risk, c1.risk, c1.corr, w1, w2)
c2.rtn = implied_r2(ratio, port.rtn, port.risk, c2.risk, c2.corr, w1, w2)

# D ==========
# Different everything

d1 = Asset(risk=0.10, corr=0.5)
d2 = Asset(risk=0.4, corr=-0.6)

d1.rtn = implied_r2(ratio, port.rtn, port.risk, d1.risk, d1.corr, w1, w2)
d2.rtn = implied_r2(ratio, port.rtn, port.risk, d2.risk, d2.corr, w1, w2)
```
...
```{python Risk terms, echo=FALSE, fig.align="center"}
term1 = lambda risk: w2**2 * risk**2
term2 = lambda risk: 2 * w1 * w2 * port.risk * risk * -0.2

def plot_risk_terms():
    results = []
    risks = np.arange(0, 0.75, 0.05)
    ...
    pd.DataFrame(results) \
        .assign(Sum = lambda frame: frame.term1 + frame.term2) \
        .plot(x="risk", lw=1, ax=ax)
    
    # Points
    assets = [a1, a2]
    points = list(map(lambda x: [x.risk, term1(x.risk) + term2(x.risk)], assets))
    
    xs = [pt[0] for pt in points]
    ys = [pt[1] for pt in points]
    
    ax.scatter(xs, ys, c="black", s=20, zorder=20)
    
    renamings = {
        "term1": "$w_a^2 \sigma_a^2$ (Term1)",
        "term2": "$2 w_p w_a \sigma_p \sigma_a \\rho$ (Term2)",
        "Sum": "Sum (Term1 + Term2)"
    }
    
    for label in  plt.legend().get_texts():
        label.set_text(renamings[label.get_text()])
        
    ax.set_xlabel("Asset Risk ($\sigma_a$)")
    ax.set_ylabel("")

    ax.set_xticklabels(["{:.1f}%".format(label * 100) for label in ax.get_xticks()])
    ax.set_yticklabels(["{:.3f}".format(label) for label in ax.get_yticks()])
...
```{python Scatter, echo=FALSE, fig.align="center"}
assets = [port, a1, a2, b1, b2, c1, c2, d1, d2]

# Now, focus on the important points
points = list(map(lambda x: [x.risk, x.rtn], assets))

xs = [pt[0] for pt in points]
ys = [pt[1] for pt in points]

fig, ax = plt.subplots()
ax.scatter(xs, ys, c="black", s=20, zorder=2)
...
ax.set_xlim([-0.031, 0.51])
ax.set_ylim([-0.06, 0.21])

ax.set_xlabel("Risk ($\sigma$)")
ax.set_ylabel("Return ($r$)")

ax.set_xticklabels(["{:.0f}%".format(label * 100) for label in ax.get_xticks()])
ax.set_yticklabels(["{:.0f}%".format(label * 100) for label in ax.get_yticks()])
...
```{python Scatter with correlation lines, echo=FALSE, fig.align="center"}
assets = [port, a1, a2, b1, b2, c1, c2, d1, d2]
...
rhos = [-0.6, -0.2, 0.4, 0.5, 0.8]
risks = np.linspace(0, 0.60, 100)
...
for rho in rhos:
    for risk in risks:
        results.append({
            "rho": rho,
            "risk": risk,
            "rtn": implied_r2(ratio, port.rtn, port.risk, risk, rho, w1, w2)
        })
        
frame = pd.DataFrame(results)
groups = frame.groupby(['rho'])

for idx, (rho, subframe) in enumerate(groups):
    subframe.plot(
        ax=ax,
        x="risk",
        y="rtn",
        label=rho,
        lw=1,
        c=colormap.jet(1 - (1 - rho) / 2),
        legend=False,
        zorder=1
    )

# Vertical label alignments
vas = ["bottom", "bottom", "top", "bottom", "bottom",]

# Apply labeling routine
label_lines(plt.gca().get_lines(), ax, [0.35]*5, vas)
...
```{python Indifference curves, echo=FALSE, fig.align="center"}
def gen_fn(): 
    r1, r2, s1, s2, p12, w1, w2, rrr1, rrr2 = symbols('r1 r2 s1 s2 p12 w1 w2 rrr1, rrr2')

    a, b = symbols('a b')

    port_rtn = ((r1 * w1) + (r2 * w2))
    port_risk = (w1**2 * s1**2 + w2**2 * s2**2 + (2 * s1 * s2 * w1 * w2 * p12))**(1/2) 

    expr1 = (port_rtn / port_risk).subs({
        w2: (1 - w1)
    })

    # a = ratio of the volatilities (s2 / s1)
    # b = ratio of the rrr's (rrr2 / rrr1)
    ...
    # Create an equation and rearrange in terms of the relative rrr
    eq1 = solve(Eq(r1 / s1, expr2), b)[0]

    # Convert the equation into a Python function
    fn1 = lambdify([a, s1, w1, p12], eq1)
    
    return fn1

fn1 = gen_fn()

def implied_b(w, correlation):
    return fn1(2, 0.15, w, correlation)

results = []
labels = []

for w in [0.0, 0.2, 0.4, 0.6, 0.8, 0.9999]:
    labels.append("$w_p={:.1f}, w_a={:.1f}$".format(w, 1 - w, 1))
    
    for rho in np.linspace(-1, 1, 41):
        results.append({
            "rho": rho,
            "w1": w,
            "b": implied_b(w, rho)
...
# Ammend the last line to emphasize underlying inequality
ln = ax.lines[-1]
ln.set_color("black")
ln.set_linestyle("dashed")

# Redraw legend to reflect changes
...
ax.set_ylabel("Relative $RRR$ ($RRR_a / RRR_p$)")
...
Even though we’ve been writing about them for years[^fn5], we know return to risk ratios and Sharpe ratios are not going anywhere.  That’s fine, but we do hope that people remain cognizant of their limitations.  Those specific limitations were made very obvious in the above preference test.  

[^fn5]: Burghardt, Walls _Managed futures for Institutional Investors (Chapter 12: Superstars versus Teamwork)_.  If you'd like a copy of this work, please email me.
...
```{python A1 A2, echo=FALSE, fig.align="center"}
assets = [a1, a2]
...
ax.plot([-0.5, 1.0], [a1.rtn, a1.rtn], c="black", lw=1, ls="dotted", zorder=1)
...
```{python Partial, echo=FALSE, fig.align="center"}
def plot_dRRR_dsa():
    R, w1, w2, r1, r2, s1, s2, p12 = symbols("R w1 w2 r1 r2 s1 s2 p12")
    
    expr1 = (w1 * r1 + w2 * r2) / sqrt(w1**2 * s1**2 + w2**2 * s2**2 + 2 * w1 * w2 * s1 * s2 * p12)
    diff1 = diff(expr1, s2)
    fn1 = lambdify([w1, w2, r1, r2, s1, s2, p12], diff1)

    def fn2(s2, w1=0.9, w2=0.1, r1=0.05, r2=0.04, s1=0.15, p12=-0.2):
        return fn1(w1, w2, r1, r2, s1, s2, p12)
    
    # Compute derivative for various s2s
    results = []

    for risk in np.arange(0, 1.0, 0.005):
        results.append({
            "risk": risk,
            "deriv": fn2(risk)
        })
    ...
    ax.set_xlabel("Prospective Asset Risk ($\sigma_a$)")
    ax.set_ylabel("$\\frac{\partial RRR}{\partial \sigma_a}$")
    ax.set_xticklabels(["{:.0f}%".format(label * 100) for label in ax.get_xticks()])
